...
def get_dataset(file_path):
    ...
    selectcols = df.loc[:, 'apk_accept':'apk_writev']
...
def vis_latent(data, labels,
               model_name="kpca_for_ad_calllog"):

    y_true = labels['apk_is_mal']
    filename = os.path.join(model_name, "kpca_mean_calllog.png")
...
if __name__ == '__main__':
    ...
    if args.weights:
        ...
        labels.to_csv ('kpca_for_ad_calllog/kpca_for_ad_calllog_scores.csv', index = False, header=True)


    else:

        print('Training mode...')
        os.makedirs('kpca_for_ad_calllog', exist_ok=True)
        ...
        filename = "kpca_for_ad_calllog/kpca_for_ad_calllog.mdl"
